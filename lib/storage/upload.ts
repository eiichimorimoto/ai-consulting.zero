...
function sanitizeFileName(filename: string): string {
  ...
  const safe = base.replace(/[^a-zA-Z0-9_-]/g, "_").substring(0, 100) // 最大100文字
...
export async function uploadFiles(
  ...
  return Promise.all(files.map((file) => uploadFile(file, userId, sessionId)))
}
...
export async function deleteFile(filePath: string): Promise<boolean> {
  try {
    ...
    const { error } = await supabase.storage.from("consulting-attachments").remove([filePath])
