...
export function getAvailableSections(messages: Message[]): AvailableSection[] {
  const chatMessages = messages.filter(
    (m) => (m.type === "ai" || m.type === "user") && m.content.trim()
  )
 ...
 * 選択されたセクションからレポートを構築
 */
export function buildReportSections(messages: Message[], sectionIds: SectionId[]): ReportSection[] {
...
function extractSwotSection(messages: Message[]): ReportSection | null {
  const swotMessage = messages.find(
    (m) =>
...
function extractTrendsSection(messages: Message[]): ReportSection | null {
  const trendsMessage = messages.find(
    (m) =>
      isAssistantMessage(m) &&
      (m.content.includes("業界動向") ||
        m.content.includes("トレンド") ||
        m.content.includes("業界"))
  )
...
function extractMarketSection(messages: Message[]): ReportSection | null {
  const marketMessage = messages.find(
    (m) => isAssistantMessage(m) && (m.content.includes("市場") || m.content.includes("マーケット"))
  )
...
function extractRecommendationSection(messages: Message[]): ReportSection | null {
  const recMessage = messages.find(
    (m) =>
      isAssistantMessage(m) &&
...
function hasTrendsAnalysis(messages: Message[]): boolean {
  return messages.some(
    (m) =>
      isAssistantMessage(m) && (m.content.includes("業界動向") || m.content.includes("トレンド"))
  )
}
    ...
    if (listOrdered === true && orderedItems.length > 0) {
      ...
      blocks.push(
        `<ul class="report-list">` +
          listItems.map((li) => `<li>${inlineMarkdownToHtml(li)}</li>`).join("") +
          `</ul>`
      )
      listItems = []
    }
...
export function buildReportSectionsFromDifyItems(items: DifyContentItem[]): ReportSection[] {
  return items.map((item) => {
    ...
    if (/^#+\s+/.test(firstLine)) {
      ...
      if (
        firstLineTitle === titleNorm ||
        firstLineTitle.startsWith(titleNorm) ||
        titleNorm.startsWith(firstLineTitle)
      ) {
