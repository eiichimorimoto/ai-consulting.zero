...
export function checkAIResult(data: {
  ...
  if (data.scores) {
    checkCount++
    const invalidScores = Object.entries(data.scores).filter(
  ...
  if (data.issues && data.issues.length > 0) {
    ...
    const issueTexts = data.issues.map(
      (i: string | { issue?: string; description?: string; text?: string }) => {
    ...
    // 課題の具体性チェック
    checkCount++
    const vagueIssues = data.issues.filter(
      (i: string | { issue?: string; description?: string; text?: string }) => {
        // issueテキストを取得
        let issueText = ""
        if (typeof i === "string") {
  ...
  if (data.metrics) {
    ...
    const invalidMetrics = numericMetrics.filter(([key, value]) => {
      ...
      if (
        ...
        ((value as number) < 0 || (value as number) > 100)
      ) {
        return true
      }
      // 時間系（ms）は0以上
      if (
        (key.toLowerCase().includes("fcp") || key.toLowerCase().includes("lcp")) &&
        (value as number) < 0
      ) {
        return true
      }
...
export function checkDBUpdate(data: {
  ...
  if (data.fields) {
    checkCount++
    const nullFields = Object.entries(data.fields).filter(
  ...
  if (data.operation === "update" && data.previousData && data.fields) {
    checkCount++
    const changedFields = Object.entries(data.fields).filter(
      ([key, value]) => data.previousData![key] !== value
    )
