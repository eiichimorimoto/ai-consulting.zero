...
describe("report-request", () => {
  ...
  describe("buildEchoReply", () => {
    ...
    it("explicitSubject があるときはそれを表題に使い「について」を付与する", () => {
      const out = buildEchoReply(
        "長期的視点での投資判断について、これをレポートにして",
        null,
        "長期的視点での投資判断"
      )
  ...
  describe("extractReportTargetReference", () => {
    ...
    it("「〇〇の内容をレポートに」から 〇〇 を抽出", () => {
      expect(extractReportTargetReference("生産性向上の提案の内容をレポートにして下さい")).toBe(
        "生産性向上の提案"
      )
    })
    ...
    it("「〇〇についてのレポート」から 〇〇 を抽出", () => {
      expect(extractReportTargetReference("現状分析についてのレポートをお願いします")).toBe(
        "現状分析"
      )
    })
    it("「〇〇について、これをレポートに」は 〇〇 を返し直前のAI回答にしない", () => {
      expect(
        extractReportTargetReference("長期的視点での投資判断について、これをレポートにして")
      ).toBe("長期的視点での投資判断")
    })
  })
  ...
  describe("findAssistantMessageByReference", () => {
    ...
    it("復唱・レポート作成しましたは対象外。実内容のメッセージを返す", () => {
      ...
      const messages = [{ content: realContent }, { content: echo }]
  ...
  describe("deriveReportTitle", () => {
    it("先頭行から見出しを抽出する（# は除去、60字超は省略）", () => {
      expect(deriveReportTitle("現状の生産工程や生産性の詳細分析について")).toBe(
        "現状の生産工程や生産性の詳細分析について"
      )
  ...
  describe("isReportCreatedContent", () => {
    it("「〇〇のレポートを作成しました。」を true とする", () => {
      expect(
        isReportCreatedContent("「この提案をレポートにして下さい」のレポートを作成しました。")
      ).toBe(true)
    })
    it("通常のAI回答は false", () => {
