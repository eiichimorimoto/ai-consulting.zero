...
export async function middleware(request: NextRequest) {
  ...
  await logToFile({
    ...
    message: "code check",
    data: { hasCode: !!code, codeValue: code?.substring(0, 20) + "...", pathname: url.pathname },
    timestamp: Date.now(),
    sessionId: "debug-session",
    runId: "run1",
    hypothesisId: "B",
  ...
  if (url.pathname === "/" && code) {
    ...
    await logToFile({
      ...
      data: { redirectUrl: redirectUrl.toString(), safeNext, codeLength: code.length },
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run1",
      hypothesisId: "B",
  ...
  if (url.pathname === "/" && !code) {
    // #region agent log
    await logToFile({
      location: "middleware.ts:34",
      message: "top page without code",
      data: { search: url.search, allParams: Object.fromEntries(url.searchParams) },
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run1",
      hypothesisId: "A",
  ...
  if (url.pathname === "/auth/callback" && !code) {
    // #region agent log
    await logToFile({
      location: "middleware.ts:29",
      message: "callback without code",
