...
export function useConsultingSession(options: UseConsultingSessionOptions) {
  ...
  const openSessions = useMemo(() => allSessions.filter((s) => s.isOpen), [allSessions])
  ...
  useEffect(() => {
    ...
    if (
      ...
      JSON.stringify(prevStateRef.current?.openSessionIds) !==
        JSON.stringify(currentState.openSessionIds)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      completedSteps > previousCompletedStepsRef.current &&
      previousCompletedStepsRef.current > 0
    ) {
  ...
  const handleSessionChange = (sessionId: string) => {
    ...
    if (session && !session.isOpen) {
      setAllSessions(
        allSessions.map((s) =>
          s.id === sessionId ? { ...s, isOpen: true, status: "active", lastUpdated: new Date() } : s
        )
      )
    }
  ...
  const handleSessionClose = (sessionId: string) => {
    ...
    if (session && session.status === "paused") {
      setAllSessions(allSessions.map((s) => (s.id === sessionId ? { ...s, isOpen: false } : s)))
  ...
  const handleRenameSession = (sessionId: string, newName: string) => {
    setAllSessions((prevSessions) =>
      prevSessions.map((session) =>
        session.id === sessionId ? { ...session, name: newName, lastUpdated: new Date() } : session
      )
    )
  ...
  const handleNewSession = () => {
    if (openSessions.length >= MAX_OPEN_TABS) {
      toast.error("タブ数の上限", {
        description: `タブは${MAX_OPEN_TABS}個までです。いずれかを閉じてから新規を開いてください。`,
      })
      return
    }
  ...
  const handleOpenSession = (sessionId: string) => {
    ...
    if (openSessions.length >= MAX_OPEN_TABS) {
      toast.error("タブ数の上限", {
        description: `タブは${MAX_OPEN_TABS}個までです。いずれかを閉じてから開いてください。`,
      })
      return
    }
  ...
  const handleTogglePin = (sessionId: string) => {
    setAllSessions(
      allSessions.map((s) => (s.id === sessionId ? { ...s, isPinned: !s.isPinned } : s))
    )
  }
    ...
    if (sessionId === activeSessionId) {
      ...
      if (remaining.length > 0) {
        ...
        if (allRemaining.length > 0) {
          ...
          setAllSessions(
            allSessions
              .map((s) => (s.id === allRemaining[0].id ? { ...s, isOpen: true } : s))
          ...
          const newSteps: ConsultingStep[] = s.steps.map((step) => ({
            ...
            status: (step.id < currentStepId
              ...
              : step.id === currentStepId
                ? "active"
                : "pending") as ConsultingStep["status"],
          }))
          return {
            ...s,
            currentStepId,
  ...
  const confirmEndSession = async () => {
    ...
    setAllSessions(allSessions.map((s) => (s.id === activeSessionId ? { ...s, ...updates } : s)))
