...
export default function DashboardClient({ profile, company, subscription }: DashboardClientProps) {
  ...
  useEffect(() => {
    lastSwotRef.current = swotAnalysis
  }, [swotAnalysis])
  useEffect(() => {
    lastIndustryTrendsRef.current = industryTrends
  }, [industryTrends])
  useEffect(() => {
    lastWorldNewsRef.current = worldNews
  }, [worldNews])
  useEffect(() => {
    lastIndustryForecastRef.current = industryForecast
  }, [industryForecast])
  ...
  const buildChangeSummary = useCallback(
    (sectionType: string, oldData: unknown, newData: unknown): string | null => {
  ...
  const fetchSectionData = async (
    ...
    try {
      ...
      const response = await fetchWithRetry(
        ...
        120_000,
        3
      ) // タイムアウト120秒、最大3回リトライ（業界予測等の重いClaude API処理対応）
      ...
      const getOldData = () => {
        switch (sectionType) {
          case "swot-analysis":
            return lastSwotRef.current
          case "industry-trends":
            return lastIndustryTrendsRef.current
          case "world-news":
            return lastWorldNewsRef.current
          case "industry-forecast":
            return lastIndustryForecastRef.current
          default:
            return null
        }
      }
      ...
      if (sectionType === "swot-analysis") {
        setSwotError(
          "SWOT分析を作成できませんでした。企業情報が不足しているか、外部データの取得に失敗した可能性があります。"
        )
      }
    } finally {
      setRefreshing((prev) => {
  ...
  const handleTabChange = async (tab: AnalysisTab) => {
    ...
    try {
      await Promise.all(sections.map((section) => fetchSectionData(section, false)))
    ...
    try {
      ...
      if (cached) {
        ...
        const loadedTabNames = Object.entries(loadedTabs)
          ...
          .map(([tab, _]) => tab)
        console.log(
          `✅ キャッシュからデータを復元しました（軽量セクション + タブ: [${loadedTabNames.join(", ")}]）`
        )
        return true
      }
    } catch (e) {
    ...
    <>
              ...
              <div style={{ position: "relative" }}>
                ...
                {settingsMenuOpen && (
                  ...
                  >
                    ...
                    <Link
                      href="/dashboard/settings?tab=account#profile-section"
                      className="nav-item"
                      style={{
                        padding: "8px 14px",
                        fontSize: "13px",
                    ...
                    <Link
                      href="/dashboard/settings?tab=account#company-section"
                      className="nav-item"
                      style={{
                        padding: "8px 14px",
                        fontSize: "13px",
                    ...
                    <Link
                      href="/dashboard/settings?tab=account#password-section"
                      className="nav-item"
                      style={{
                        padding: "8px 14px",
                        fontSize: "13px",
          ...
          >
            ...
            >
              <div
                style={{
                  background:
                    "linear-gradient(135deg, rgba(248, 250, 252, 0.08) 0%, rgba(226, 232, 240, 0.05) 50%, rgba(241, 245, 249, 0.08) 100%)",
              ...
              >
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  >
                    {showNewGreeting ? "はじめまして！" : "おかえりなさい"}
                  </p>
                  <h1
                    style={{
                      fontSize: "18px",
          ...
          <div className="content" style={{ paddingTop: updateMessage ? "220px" : "180px" }}>
            ...
            {localInfo ? (
              localInfo.traffic && localInfo.traffic.length > 0 ? (
                      ...
                      {[...localInfo.traffic, ...localInfo.traffic].map((item, idx) => (
                        <span key={idx} className="traffic-ticker-item">
                          ...
                          >
                            {item.title}
                            <span
                              className={`traffic-status-text ${item.status === "warning" ? "status-warning" : item.status === "error" ? "status-error" : "status-normal"}`}
                            >
                              [
                              {item.status === "warning"
                                ? "遅延"
                ...
                <div
                  className="traffic-ticker-container"
                ...
                >
                  ...
                  >
                    <span>🟢</span>
                    <span>
                      現在、特別な交通情報はありません（{company?.location || "エリア"}周辺）
                    </span>
                  </div>
                </div>
              )
                ...
                {loading ? (
                  ...
                  <>
                    <div className="market-card">
                      <div className="market-card-header">
                        ...
                        {marketData?.usdJpy && marketData.usdJpy.length > 1 && (
                          <span
                            className={`market-change ${marketData.usdJpy[marketData.usdJpy.length - 1].value > marketData.usdJpy[0].value ? "up" : "down"}`}
                          >
                            {(
                              (marketData.usdJpy[marketData.usdJpy.length - 1].value /
                                marketData.usdJpy[0].value -
                                1) *
                              100
                            ).toFixed(2)}
                            %
                      ...
                      <div className="market-value">
                        ¥
                        {marketData?.usdJpy?.[marketData.usdJpy.length - 1]?.value.toFixed(2) ||
                          "156.42"}
                      </div>
                        ...
                        {marketData?.usdJpy ? (
                          ...
                          <LineChart
                            ...
                            data={weeks.map((w, i) => {
                              ...
                              return { value: 154.2 + i * 0.3, week: w, date: d.toISOString() }
                            })}
                            options={{ prefix: "¥", lineColor: "#6366F1" }}
                          />
                        )}
                    ...
                    <div className="market-card">
                      <div className="market-card-header">
                        ...
                        {marketData?.nikkei && marketData.nikkei.length > 1 && (
                          <span
                            className={`market-change ${marketData.nikkei[marketData.nikkei.length - 1].value > marketData.nikkei[0].value ? "up" : "down"}`}
                          >
                            {(
                              (marketData.nikkei[marketData.nikkei.length - 1].value /
                                marketData.nikkei[0].value -
                                1) *
                              100
                            ).toFixed(2)}
                            %
                      ...
                      <div className="market-value">
                        ¥
                        {marketData?.nikkei?.[
                          marketData.nikkei.length - 1
                        ]?.value.toLocaleString() || "39,847"}
                      </div>
                      <div className="chart-container">
                        {marketData?.nikkei ? (
                          <LineChart
                    ...
                    <div className="market-card">
                      <div className="market-card-header">
                        ...
                        {marketData?.longRate && marketData.longRate.length > 1 && (
                          <span
                            className={`market-change ${marketData.longRate[marketData.longRate.length - 1].value > marketData.longRate[0].value ? "up" : "down"}`}
                          >
                            {(
                              (marketData.longRate[marketData.longRate.length - 1].value /
                                marketData.longRate[0].value -
                                1) *
                              100
                            ).toFixed(2)}
                            %
                      ...
                      <div className="market-value">
                        {marketData?.longRate?.[marketData.longRate.length - 1]?.value.toFixed(3) ||
                          "1.085"}
                        %
                      </div>
                      <div className="chart-container">
                        {marketData?.longRate ? (
                          <LineChart
                            ...
                            data={marketData.longRate.map((d) => ({
                              value: d.value,
                              week: d.week,
                              date: d.date || d.week,
                            }))}
                    ...
                    <div className="market-card">
                      <div className="market-card-header">
                        ...
                        {marketData?.shortRate && marketData.shortRate.length > 1 && (
                          <span
                            className={`market-change ${marketData.shortRate[marketData.shortRate.length - 1].value > marketData.shortRate[0].value ? "up" : "down"}`}
                          >
                            {(
                              (marketData.shortRate[marketData.shortRate.length - 1].value /
                                marketData.shortRate[0].value -
                                1) *
                              100
                            ).toFixed(2)}
                            %
                      ...
                      <div className="market-value">
                        {marketData?.shortRate?.[marketData.shortRate.length - 1]?.value.toFixed(
                          2
                        ) || "0.25"}
                        %
                      </div>
                      <div className="chart-container">
                        {marketData?.shortRate ? (
                          <LineChart
                            ...
                            data={marketData.shortRate.map((d) => ({
                              value: d.value,
                              week: d.week,
                              date: d.date || d.week,
                            }))}
                          ...
                          <LineChart
                            ...
                            data={weeks.map((w, i) => {
                              const d = new Date()
                              d.setDate(d.getDate() - (7 - i) * 7)
                              return { value: 0.1 + i * 0.02, week: w, date: d.toISOString() }
                            })}
                            options={{ unit: "%", lineColor: "#F59E0B" }}
                          />
                        )}
                    ...
                    <div className="market-card" style={{ gridColumn: "1 / -1" }}>
                      ...
                      >
                        {marketData?.commodities &&
                        Array.isArray(marketData.commodities) &&
                              ...
                              >
                                ...
                                <div
                                  style={{
                                    fontSize: "10px",
                                    color:
                                      (c.change || 0) >= 0 ? "var(--success)" : "var(--danger)",
                                    marginTop: "2px",
                                  }}
                                >
                                  {(c.change || 0) >= 0 ? "↑" : "↓"}{" "}
                                  {Math.abs(c.change || 0).toFixed(1)}%
                                </div>
                              </div>
                            ))
                          : // フォールバック表示（デフォルト原材料）
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                      <span style={{ fontSize: "28px", fontWeight: "700" }}>
                        {localInfo?.laborCosts?.comparison?.industryAverage?.toLocaleString() ||
                          "1,280"}
                      </span>
                      ...
                      <span
                        style={{
                          ...
                          borderRadius: "2px",
                          background:
                            (localInfo?.laborCosts?.change || 2.5) >= 0
                              ? "rgba(255,255,255,0.2)"
                              : "rgba(239,68,68,0.3)",
                          marginLeft: "auto",
                        }}
                      >
                        前年比 {(localInfo?.laborCosts?.change || 2.5) >= 0 ? "+" : ""}
                        {localInfo?.laborCosts?.change || 2.5}%
                      </span>
                    </div>
                    <div
                      style={{ fontSize: "9px", opacity: 0.8, marginTop: "6px", lineHeight: "1.4" }}
                    >
                      ※ {company?.prefecture || "愛知県"}内の{company?.industry || "製造業"}における
                      <br />
                      パート・アルバイトの平均募集時給です
                    </div>
                  </div>
                  <div className="local-content" style={{ fontSize: "10px", lineHeight: "1.4" }}>
                    ...
                    >
                      <span style={{ fontWeight: 600 }}>
                        {company?.prefecture || "愛知県"}最低賃金
                      </span>
                      <span style={{ fontWeight: "700", color: "var(--text-primary)" }}>
                        {localInfo?.laborCosts?.comparison?.minimumWage?.toLocaleString() ||
                          "1,077"}
                        円
                    ...
                    >
                      ...
                      >
                        💰 {company?.prefecture || "愛知県"}・{company?.industry || "製造業"}
                        の平均給与
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                        ...
                        >
                          ...
                          <div style={{ fontSize: "14px", fontWeight: "700", color: "#166534" }}>
                            {(localInfo?.laborCosts as any)?.monthly?.toLocaleString() ||
                              (
                                localInfo?.laborCosts as any
                              )?.comparison?.industryMonthly?.toLocaleString() ||
                              "28.5"}
                            万円
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <div style={{ fontSize: "14px", fontWeight: "700", color: "#166534" }}>
                            {(localInfo?.laborCosts as any)?.yearly?.toLocaleString() ||
                              (
                                localInfo?.laborCosts as any
                              )?.comparison?.industryYearly?.toLocaleString() ||
                              "420"}
                            万円
                          </div>
                        </div>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                        ...
                        >
                          ...
                          <div style={{ fontSize: "14px", fontWeight: "700", color: "#0369a1" }}>
                            {localInfo?.laborCosts?.comparison?.industryHourlyRange?.min?.toLocaleString() ||
                              "1,100"}
                            円
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <div style={{ fontSize: "14px", fontWeight: "700", color: "#0369a1" }}>
                            {localInfo?.laborCosts?.comparison?.industryHourlyRange?.max?.toLocaleString() ||
                              "1,600"}
                            円
                          </div>
                        </div>
                      ...
                      {(() => {
                        ...
                        for (let d = 1; d <= lastDay.getDate(); d++) {
                          ...
                          const hasEvent = localInfo?.events?.some((e: any) => {
                            const eventMonth =
                              e.date?.match(/(\d+)月/)?.[1] || e.date?.match(/^(\d+)\//)?.[1]
                            const eventDay =
                              e.date?.match(/(\d+)日/)?.[1] ||
                              e.date?.match(/\/(\d+)/)?.[1] ||
                              e.date?.match(/(\d+)/)?.[1]
                            const matchMonth = eventMonth
                              ? parseInt(eventMonth) === calendarMonth.getMonth() + 1
                              : true
                      ...
                      localInfo.events.slice(0, 5).map((event, idx) => (
                        ...
                        >
                          <span
                            style={{
                              background:
                                ["#10b981", "#3b82f6", "#f59e0b", "#8b5cf6", "#ec4899"][idx] ||
                                "#64748b",
                              color: "white",
                              padding: "2px 6px",
                              borderRadius: "2px",
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <div style={{ fontSize: "40px" }}>
                        {localInfo?.weather?.current?.icon || "☀️"}
                      </div>
                      <div style={{ flex: 1 }}>
                ...
                <div className="debug-panel-content">
                  <div className="debug-section">
                    ...
                    <p
                      style={{
                        fontSize: "12px",
                        color: localInfo._debug.apiKeyConfigured
                          ? "var(--success)"
                          : "var(--danger)",
                      }}
                    >
                  ...
                  {localInfo._debug.laborCosts && (
                    <div className="debug-section">
                      ...
                      <details>
                        ...
                        <ul>
                          {localInfo._debug.laborCosts.searchQueries?.map(
                            (q: string, i: number) => (
                        ...
                        {localInfo._debug.laborCosts.searchLogs?.map((log: any, i: number) => (
                          ...
                          >
                            ...
                            <p style={{ fontSize: "11px", color: "var(--text-secondary)" }}>
                              結果数: {log.resultCount || 0} / 検証済み:{" "}
                              {log.verifiedCount || log.results?.length || 0}
                            </p>
                            {log.results && log.results.length > 0 && (
                              <details style={{ marginTop: "4px" }}>
                                ...
                                {log.results.map((r: any, j: number) => (
                                  <div
                                    key={j}
                                    style={{ marginTop: "4px", padding: "4px", fontSize: "11px" }}
                                  >
                                    <p>
                  ...
                  {localInfo._debug.events && (
                    <div className="debug-section">
                      ...
                      <p>
                        結果数: {localInfo._debug.events.resultCount || 0} / 検証済み:{" "}
                        {localInfo._debug.events.verifiedCount ||
                          localInfo._debug.events.allResults?.length ||
                          0}
                      </p>
                      <details>
                  ...
                  {localInfo._debug.infrastructure && (
                    <div className="debug-section">
                      <h4>インフラ情報検索</h4>
                      <p>
                        検索クエリ数: {localInfo._debug.infrastructure.searchQueries?.length || 0}
                      </p>
                      <p>総結果数: {localInfo._debug.infrastructure.totalResults || 0}</p>
                      <p style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
                        検証済み結果数:{" "}
                        {localInfo._debug.infrastructure.searchLogs?.reduce(
                          (sum: number, log: any) => sum + (log.verifiedCount || 0),
                          0
                        ) || 0}
                      </p>
                      <details>
                  ...
                  {localInfo._debug.weather && (
                    <div className="debug-section">
                      ...
                      <p>
                        結果数: {localInfo._debug.weather.resultCount || 0} / 検証済み:{" "}
                        {localInfo._debug.weather.verifiedCount ||
                          localInfo._debug.weather.searchResults?.length ||
                          0}
                      </p>
                      <details>
                        <summary>検索結果</summary>
            ...
            >
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                    {[
                      ...
                      { key: "world-news", label: "世界ニュース", r: refreshing["world-news"] },
                      {
                        ...
                        label: "業界予測",
                        r: refreshing["industry-forecast"],
                      },
                    ].map(({ key, label, r }) => (
                      <span
                        key={key}
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    borderBottom:
                      activeTab === "swot" ? "3px solid var(--primary)" : "3px solid transparent",
                    cursor: "pointer",
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <svg
                      ...
                      style={{
                        width: "14px",
                        height: "14px",
                        stroke: activeTab === "swot" ? "white" : "currentColor",
                        fill: "none",
                        strokeWidth: 2,
                      }}
                    >
                  ...
                  style={{
                    ...
                    border:
                      activeTab === "trends"
                        ...
                        : "1px solid rgba(148, 163, 184, 0.2)",
                    borderBottom:
                      activeTab === "trends" ? "3px solid var(--primary)" : "3px solid transparent",
                    cursor: "pointer",
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <svg
                      ...
                      style={{
                        width: "14px",
                        height: "14px",
                        stroke: activeTab === "trends" ? "white" : "currentColor",
                        fill: "none",
                        strokeWidth: 2,
                      }}
                    >
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <svg
                      ...
                      style={{
                        width: "14px",
                        height: "14px",
                        stroke: activeTab === "forecast" ? "white" : "currentColor",
                        fill: "none",
                        strokeWidth: 2,
                      }}
                    >
                  ...
                  style={{
                    ...
                    border:
                      activeTab === "news"
                        ...
                        : "1px solid rgba(148, 163, 184, 0.2)",
                    borderBottom:
                      activeTab === "news" ? "3px solid var(--primary)" : "3px solid transparent",
                    cursor: "pointer",
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <svg
                      ...
                      style={{
                        width: "14px",
                        height: "14px",
                        stroke: activeTab === "news" ? "white" : "currentColor",
                        fill: "none",
                        strokeWidth: 2,
                      }}
                    >
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              ...
                              >
                                {trend.title?.slice(0, 20) || trend.category}
                              </span>
                              <span
                                style={{
                                  fontSize: "9px",
                  ...
                  {industryForecast ? (
                    <div style={{ marginTop: "8px" }}>
                      {industryForecast.indicators && industryForecast.indicators.length > 0 && (
                        <div>
                          {industryForecast.indicators
                            .slice(0, 5)
                            .map((ind: ForecastIndicator, idx: number) => (
                              <div
                                key={idx}
                                style={{
                                  display: "flex",
                              ...
                              >
                                ...
                                >
                                  <span
                                    style={{ fontSize: "16px", color: "white", fontWeight: "700" }}
                                  >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  >
                                    {ind.forecast}
                                    {(ind as any).description && `。${(ind as any).description}`}
                                  </div>
                                </div>
                              </div>
                            ))}
              ...
              <div
                style={{
                  marginBottom: "20px",
                  order: 1,
                  display: activeTab === "swot" ? "block" : "none",
                }}
              >
                <div
                  id="swot-analysis-section"
                  className="analysis-card"
                  style={{ position: "relative" }}
                >
                  <div className="analysis-card-header">
                      ...
                      <>
                        ...
                        >
                          ...
                          <p style={{ margin: "0 0 8px 0", color: "var(--text-secondary)" }}>
                            <strong>強み（Strengths）</strong>: 会社の優位性や技術力
                            <br />
                            <strong>弱み（Weaknesses）</strong>: 課題や改善点
                            <br />
                            <strong>機会（Opportunities）</strong>: 市場の成長機会
                            <br />
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            height: "2px",
                            background:
                              "linear-gradient(90deg, #10b981 0%, #10b981 50%, #f59e0b 50%, #f59e0b 100%)",
                            zIndex: 5,
                          }}
                        />
                        <div
                          style={{
                            ...
                            width: "2px",
                            background:
                              "linear-gradient(180deg, #10b981 0%, #3b82f6 50%, #3b82f6 50%, #f59e0b 100%)",
                            zIndex: 5,
                          }}
                        />
                              ...
                              {swotAnalysis.competitors
                                ...
                                .map((c: Competitor, i: number) => (
                                  ...
                                  >
                                    ...
                                    {c.reason && (
                                      <div style={{ color: "var(--text-light)", fontSize: "10px" }}>
                                        理由: {c.reason}
                                      </div>
                                    )}
                                  </div>
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: "11px", color: "var(--text-secondary)" }}>
                                  総合: {swotAnalysis.reputation.overall}
                                </div>
                              </div>
                            </div>
                              ...
                              >
                                ...
                                {swotAnalysis.reputation.positives?.slice(0, 5).map((p, i) => {
                                  const comment =
                                    typeof p === "string"
                                      ? p
                                      : typeof p === "object" && p !== null && "comment" in p
                                        ? String(p.comment)
                                        : "情報なし"
                                  return (
                                    <div
                              ...
                              >
                                ...
                                {swotAnalysis.reputation.negatives?.slice(0, 5).map((n, i) => {
                                  const comment =
                                    typeof n === "string"
                                      ? n
                                      : typeof n === "object" && n !== null && "comment" in n
                                        ? String(n.comment)
                                        : "情報なし"
                                  return (
                                    <div
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                                    {swotAnalysis.reputation.negatives?.slice(0, 5).map((n, i) => {
                                      const source =
                                        typeof n === "object" && n !== null && "source" in n
                                          ? String(n.source)
                                          : "出典情報なし"
                                      return (
                                        <div
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            >
                              {[1, 2, 3, 4, 5].map((level) => {
                                ...
                                const riskLevel =
                                  riskCount === 0 ? 1 : Math.min(5, Math.max(1, riskCount))
                                return (
                                  <div
                                    key={level}
                                    style={{
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              ...
                              <div
                                style={{ fontSize: "10px", color: "#94a3b8", marginBottom: "10px" }}
                              >
                              ...
                              >
                                ...
                                >
                                  {[
                                    ...
                                    65,
                                    72,
                                    ind.trend === "up" ? 90 : ind.trend === "down" ? 25 : 55,
                                  ].map((h, i) => (
                                    <div
                                      key={i}
                                      style={{
                        ...
                        {(() => {
                          ...
                          return (
                            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                              {industryForecast.recommendation
                                .split(/[。]/)
                                .filter((s) => s.trim() && s.trim().length > 5)
                                .slice(0, 3)
                                .map((item, idx) => {
                                  const categoryMatch = item.match(/【(.+?)】/)
                                  const category = categoryMatch
                                    ? categoryMatch[1]
                                    : ["業務効率化", "収益改善", "リスク対策"][idx]
                                  ...
                                  const actionTitle =
                                    colonIdx !== -1
                                      ? cleanText.substring(0, colonIdx).trim()
                                      : cleanText
                                  const scheduleDetail =
                                    colonIdx !== -1 ? cleanText.substring(colonIdx + 1).trim() : ""
                                  ...
                                  const durationQuarters = Math.max(
                                    1,
                                    endQuarterNum - startQuarterNum + 1
                                  )
                                  const startIdx = Math.max(0, startQuarterNum - currentQuarterNum)
                                  const barLeft = (startIdx / 4) * 100
                                  const barWidth =
                                    (Math.min(durationQuarters, 4 - startIdx) / 4) * 100

                                  const configs = [
                                    {
                                      bg: "linear-gradient(135deg, rgba(99, 102, 241, 0.08), rgba(139, 92, 246, 0.04))",
                                    ...
                                    {
                                      bg: "linear-gradient(135deg, rgba(14, 165, 233, 0.06), rgba(6, 182, 212, 0.03))",
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <div style={{ position: "relative" }}>
                                          ...
                                          >
                                            <span>
                                              開始: {startYear}年{startMonth}月
                                            </span>
                                            <span style={{ color: cfg.accent, fontWeight: "600" }}>
                                              完了: {endYear}年{endMonth}月
                                            </span>
                                          </div>
                                        </div>
                                      </div>
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              transition: "all 0.25s ease",
                              boxShadow:
                                "0 2px 8px rgba(99, 102, 241, 0.06), inset 0 1px 0 rgba(255,255,255,0.6)",
                            ...
                            onMouseOver={(e) => {
                              ...
                              e.currentTarget.style.boxShadow =
                                "0 4px 12px rgba(99, 102, 241, 0.12), inset 0 1px 0 rgba(255,255,255,0.8)"
                        ...
                        >
                          <div
                            style={{
                              fontSize: "11px",
                              color:
                                industryForecast.midTerm?.outlook === "positive"
                                  ? "#10b981"
                                  : industryForecast.midTerm?.outlook === "negative"
                                    ? "#ef4444"
                                    : "#f59e0b",
                              marginBottom: "8px",
                              fontWeight: "600",
      ...
      <div className={`notifications-panel ${notificationsOpen ? "open" : ""}`}>
        <div className="notifications-panel-header">
          ...
          <button className="notifications-close-btn" onClick={() => setNotificationsOpen(false)}>
            <svg
              viewBox="0 0 24 24"
              style={{
                width: "20px",
      ...
      {/* デバッグパネルのオーバーレイ */}
      {debugPanelOpen && (
        <div className="notifications-overlay" onClick={() => setDebugPanelOpen(false)} />
      )}
    </>
  )
}
