...
export default function FactCheckBadge({
  ...
  if (compact) {
    return (
      <Badge
        className={`${getLevelColor(result.level)} flex cursor-help items-center gap-1`}
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {getLevelIcon(result.level)}
              <span className="text-sm font-semibold">{getLevelLabel(result.level)}</span>
          ...
          <p className="mt-1 text-left text-xs opacity-80">{result.summary}</p>
        </CollapsibleTrigger>
              ...
              {result.checks.map((check, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className={check.passed ? "text-gray-700" : "font-medium text-gray-900"}>
                      {check.message}
                    </p>
                    {check.suggestion && (
                      <p className="mt-1 text-blue-600">💡 {check.suggestion}</p>
                    )}
                  </div>
                  {check.passed ? (
