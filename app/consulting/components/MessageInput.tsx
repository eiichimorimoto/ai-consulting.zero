...
export function MessageInput({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (oversizedFiles.length > 0) {
        alert(
          `以下のファイルはサイズが大きすぎます（10MB以下にしてください）:\n${oversizedFiles.map((f) => f.name).join("\n")}`
        )
        e.target.value = ""
        return
      }
      ...
      if (invalidFiles.length > 0) {
        alert(
          `以下のファイルは対応していない形式です（対応: .txt, .csv, .md, .pdf, .doc, .docx, .xls, .xlsx, .ppt, .pptx）:\n${invalidFiles.map((f) => f.name).join("\n")}`
        )
        e.target.value = ""
        return
      }
  ...
  return (
    <div className="bg-background/95 supports-[backdrop-filter]:bg-background/60 h-[200px] shrink-0 border-t px-4 pb-3 pt-3 backdrop-blur">
      <div className="mx-auto max-w-4xl">
        ...
        {showError && !hasSession && (
          <div className="bg-destructive/10 text-destructive mb-2 flex items-center gap-2 rounded-md px-3 py-2 text-sm">
            ...
            <span>
              左メニューからカテゴリーを選択するか、相談履歴から既存の相談を選択してください。
            </span>
          </div>
        )}
        ...
        <div>
          <Textarea
            ...
            disabled={isLoading}
            className={`max-h-[200px] min-h-[80px] w-full resize-none text-lg transition-colors ${
              isDragging ? "bg-primary/5 border-primary border-2" : ""
          ...
          <div className="mt-2 flex items-end gap-3">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="text-muted-foreground text-[10px]">添付</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <Button
                variant="ghost"
              ...
              <span className="text-muted-foreground text-[10px]">音声</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <Button
                onClick={onSend}
              ...
              <span className="text-muted-foreground text-[10px]">送信</span>
            </div>
          </div>
        </div>
