...
export async function GET(request: Request) {
  ...
  if (urlError) {
    ...
    // #region agent log
    await logToFile({
      location: "app/auth/callback/route.ts:35",
      message: "error parameter detected",
      data: { error: urlError, errorDescription },
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run1",
      hypothesisId: "C",
  ...
  const next =
    requestedNext && requestedNext.startsWith("/") ? requestedNext : "/auth/complete-profile"

  // #region agent log
  await logToFile({
    location: "app/auth/callback/route.ts:12",
  ...
  if (!code) {
    ...
    await logToFile({
      ...
      message: "no code redirecting to login",
      data: {},
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run1",
  ...
  // #region agent log
  await logToFile({
    location: "app/auth/callback/route.ts:28",
    message: "before exchangeCodeForSession",
  ...
  // #region agent log
  await logToFile({
    location: "app/auth/callback/route.ts:30",
  ...
  if (error) {
    // #region agent log
    await logToFile({
      location: "app/auth/callback/route.ts:33",
      message: "exchange error",
      data: { errorMessage: error.message, errorStatus: error.status, errorName: error.name },
      timestamp: Date.now(),
      sessionId: "debug-session",
      runId: "run1",
      hypothesisId: "C",
  ...
  // #region agent log
  await logToFile({
    location: "app/auth/callback/route.ts:44",
    message: "session exchange success",
    data: {
      userId: data?.user?.id,
  ...
  const isProfileIncomplete =
    !profile || !profile.name || profile.name === "User" || !profile.company_id
  ...
  if (isProfileIncomplete) {
    ...
    // #region agent log
    await logToFile({
      location: "app/auth/callback/route.ts:66",
  ...
  // #region agent log
  await logToFile({
    location: "app/auth/callback/route.ts:75",
    message: "redirecting to final destination",
    data: { finalNext, userId: data?.user?.id },
    timestamp: Date.now(),
    sessionId: "debug-session",
    runId: "run1",
    hypothesisId: "C",
