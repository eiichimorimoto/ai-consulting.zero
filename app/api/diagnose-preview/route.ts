...
async function analyzeWithPageSpeed(url: string) {
  ...
  if (!apiKey || apiKey.trim().length === 0) {
    const errorMessage =
      "PageSpeed APIキーが設定されていません。Vercelの環境変数に GOOGLE_PAGESPEED_API_KEY を設定してください。"
    console.error("❌ PageSpeed APIキーが設定されていません:", {
      hasKey,
      keyLength,
      nodeEnv: process.env.NODE_ENV,
      allEnvKeys: Object.keys(process.env).filter(
        (k) => k.includes("PAGESPEED") || k.includes("GOOGLE")
      ),
    })
  ...
  if (trimmedKey.length === 0) {
    ...
    throw new Error(
      "PageSpeed APIキーが空です。Vercelの環境変数 GOOGLE_PAGESPEED_API_KEY を確認してください。"
    )
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: `無効なURL形式です: ${url}` }, { status: 400 })
    }
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Claude APIを使用するには、環境変数 ANTHROPIC_API_KEY を設定する必要があります。",
...
`
    ...
    if (
      err?.message?.includes("PageSpeed APIキー") ||
      err?.message?.includes("PageSpeed API key") ||
      err?.message?.includes("GOOGLE_PAGESPEED_API_KEY")
    ) {
      return NextResponse.json(
        {
          error: "PageSpeed APIキーが設定されていません",
          details:
            "Vercelの環境変数に GOOGLE_PAGESPEED_API_KEY を設定してください。設定後、デプロイを再実行してください。",
    ...
    if (
      err?.message?.includes("利用制限に達しました") ||
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Google PageSpeed APIは1日あたり25,000リクエストの制限があります。数分後に再度お試しください。",
