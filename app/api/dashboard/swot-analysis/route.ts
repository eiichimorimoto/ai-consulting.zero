...
const swotSchema = z.object({
  ...
  reputation: z.object({
    ...
    })).min(3).max(3).describe("良い評判（必ず3項目）"),
    negatives: z.array(z.object({
      comment: z.string().describe("悪い評判の内容（30文字以内）"),
      source: z.string().describe("出典（URL、サイト名、またはプラットフォーム名）"),
    })).min(2).max(2).describe("悪い評判（必ず2項目）"),
  }).describe("SNS/口コミ評判（良い評判3つ + 悪い評判2つ = 合計必ず5項目）"),
})
...
7. SNS/口コミ評判（合計5項目必須）
   - 【必須】良い評判を必ず3項目生成すること（2項目以下は不可）
   - 【必須】悪い評判を必ず2項目生成すること（1項目以下は不可）
   - 各項目30文字以内
   - 【重要】各評判に必ず出典を明記（URLまたはサイト名）
   - 出典は検索結果から実際に見つかったものを使用
   - 情報が不足している場合は「情報不足」ではなく、検索結果から推測して5項目すべてを埋めること
