...
async function getMarketData(loginDate: Date) {
  ...
  if (nikkeiData) {
    console.log(
      `✅ 日経平均: ${currentNikkei}円 (${nikkeiData.change > 0 ? "+" : ""}${nikkeiData.change})`
    )
  } else {
...
const industryMaterials: Record<
  string,
  { key: string; name: string; unit: string; basePrice: number; volatility: number }[]
> = {
  // 製造業
  製造業: [
...
function getRelevantMaterials(
  ...
  businessDesc: string,
  retrievedInfo?: any
): { materials: (typeof industryMaterials)["default"]; matchedCategory: string } {
  ...
  for (const keyword of consultingKeywords) {
    if (searchText.includes(keyword)) {
      ...
      return {
        materials: industryMaterials["コンサルティング業"],
        matchedCategory: "コンサルティング業",
      }
    }
  }
  ...
  if (
    ...
    industryLower.includes("ソフトウェア") ||
    industryLower.includes("システム") ||
    descLower.includes("システム開発") ||
    descLower.includes("アプリ") ||
    descLower.includes("web") ||
    descLower.includes("クラウド")
  ) {
  ...
  // 食品製造・加工
  if (
    industryLower.includes("食品") ||
    descLower.includes("食品") ||
    descLower.includes("食材") ||
    descLower.includes("加工食品")
  ) {
  ...
  // 飲料
  if (
    industryLower.includes("飲料") ||
    descLower.includes("飲料") ||
    descLower.includes("ドリンク") ||
    descLower.includes("ジュース")
  ) {
  ...
  // 建設業
  if (
    industryLower.includes("建設") ||
    industryLower.includes("建築") ||
    descLower.includes("建設") ||
    descLower.includes("施工") ||
    descLower.includes("土木")
  ) {
  ...
  // 不動産
  if (
    industryLower.includes("不動産") ||
    descLower.includes("不動産") ||
    descLower.includes("賃貸") ||
    descLower.includes("マンション")
  ) {
  ...
  // 小売業
  if (
    industryLower.includes("小売") ||
    descLower.includes("小売") ||
    descLower.includes("販売店") ||
    descLower.includes("ショップ") ||
    descLower.includes("店舗")
  ) {
  ...
  // 卸売業
  if (
    industryLower.includes("卸売") ||
    descLower.includes("卸売") ||
    descLower.includes("商社") ||
    descLower.includes("問屋")
  ) {
  ...
  // 運輸・物流
  if (
    industryLower.includes("運輸") ||
    industryLower.includes("物流") ||
    descLower.includes("運送") ||
    descLower.includes("物流") ||
    descLower.includes("配送") ||
    descLower.includes("トラック")
  ) {
  ...
  // 医療
  if (
    industryLower.includes("医療") ||
    descLower.includes("医療") ||
    descLower.includes("病院") ||
    descLower.includes("クリニック") ||
    descLower.includes("診療所")
  ) {
  ...
  // 製造業
  if (
    industryLower.includes("製造") ||
    descLower.includes("製造") ||
    descLower.includes("工場") ||
    descLower.includes("メーカー")
  ) {
...
function generateCommodityPrices(
  materials: (typeof industryMaterials)["default"],
  currentRate: number
) {
  return materials.map((m) => ({
    ...
    priceJpy: m.unit.includes("円")
      ? Math.round(m.basePrice + (Math.random() - 0.5) * m.volatility * 2)
      : Math.round((m.basePrice + (Math.random() - 0.5) * m.volatility * 2) * currentRate),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { materials: relevantMaterials, matchedCategory } = getRelevantMaterials(
      companyIndustry,
      companyBusinessDesc,
      companyRetrievedInfo
    )
    console.log(
      "🏭 マッチした業種カテゴリ:",
      matchedCategory,
      "→ 原材料:",
      relevantMaterials.map((m) => m.name).join(", ")
    )
