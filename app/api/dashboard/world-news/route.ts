...
export async function GET(request: Request) {
  try {
    ...
    const { data: company } = await supabase
      .from('companies')
      .select('industry, business_description, main_products')
    ...
    // 業界・製品に影響するニュースを検索（業種を明確に含める）
    const industryQuery = company.industry || '製造業'
    const queries = [
      `${industryQuery} 経済 ニュース 2025 影響`,
      `${industryQuery} 紛争 影響 リスク`,
      `${industryQuery} 政策 変更 規制`,
      `${industryQuery} 業界 動向 ニュース`,
      ...(company.main_products || []).slice(0, 2).map((product: string) => 
        `${industryQuery} ${product} 市場 動向 2025`
      ),
    ]
