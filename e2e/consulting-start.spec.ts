...
test.describe("Start画面", () => {
  ...
  test("初期表示で「新規」「既存」ボタンが表示され、タブは表示されない", async ({ page }) => {
    ...
    await expect(tabLike)
      .not.toBeVisible({ timeout: 2000 })
      .catch(() => {})
  })

  test("「新規」クリックでタブが1つ出現し、チャットエリアにヘッダーとメッセージが表示される", async ({
    page,
  }) => {
    const url = page.url()
    if (!url.includes("/consulting/start")) {
    ...
    const hasTabOrLabel =
      ...
      (await page
        ...
        .catch(() => false)) ||
      (await page
        .locator('button:has-text("新規相談")')
        .first()
        .isVisible()
        .catch(() => false))

    // チャットエリア: ヘッダー「課題のヒアリング」またはAIメッセージが表示される
    const hasChatHeader = await page
      .locator("text=課題のヒアリング")
      .isVisible()
      .catch(() => false)
    const hasAiMessage =
      (await page
    ...
    expect(
      hasTabOrLabel || hasChatHeader || hasAiMessage,
      "新規クリック後にタブ/チャットが表示されること"
    ).toBeTruthy()
  })
